...
// a) Через цикл for
const numbers = [];
for(let i = 0; i <= 30; i += 1)numbers.push(i);
console.log(numbers);
// b) Через Array.from()
...
// b) Через Array.from()
const createRange1 = (start, end)=>Array.from({
        length: end - start + 1
    }, (_, index)=>start + index);
console.log(createRange1(5, 25));
// c) Через .fill() + .map()
const createRangeFillMap = (start, end)=>new Array(end - start + 1).fill(0).map((_, index)=>start + index);
console.log(createRangeFillMap(5, 25));

//# sourceMappingURL=js-task.c7e7cbc5.js.map
